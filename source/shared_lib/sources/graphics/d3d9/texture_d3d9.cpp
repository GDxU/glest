		...
		if(pixmapInit){
			for(int i=0; i<6; ++i){	
				...
				if(currentFace->getW()!=l || currentFace->getH()!=l){
					throw std::runtime_error("Can't create Direct3D cube texture: dimensions don't agree");
				}
				if(currentFace->getComponents()!=components){
					throw std::runtime_error("Can't create Direct3D cube texture: components don't agree");
				}
			}
		}
