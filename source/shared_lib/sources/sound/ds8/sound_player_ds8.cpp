...
void SoundBuffer::createDsBuffer(IDirectSound8 *dsObject){
	...
	if (hr!=DS_OK){
        throw std::runtime_error("Failed to create direct sound buffer");
	}
    ...
    buffer->Release();
	if (hr!=S_OK){
        throw std::runtime_error("Failed to create direct sound 8 static buffer");
	}
}
...
void StaticSoundBuffer::fillDsBuffer(){
	...
	if (hr!=DS_OK){
        throw std::runtime_error("Failed to Lock direct sound buffer");
	}
	...
	if (hr!=DS_OK){
        throw std::runtime_error("Failed to Unlock direct sound buffer");
	}
}
...
void StrSoundBuffer::refreshDsBuffer(){
	...
	if(hr!=DS_OK){
		throw std::runtime_error("Failed to Lock query play position");
	}
...
void SoundPlayerDs8::init(const SoundPlayerParams *params){
	...
	if (hr!=DS_OK){
        throw std::runtime_error("Can't create direct sound object");
	}
    
	//Set cooperative level
    hr= dsObject->SetCooperativeLevel(GetActiveWindow(), DSSCL_PRIORITY); 
	if (hr!=DS_OK){
        throw std::runtime_error("Can't set cooperative level of dsound");
	}
